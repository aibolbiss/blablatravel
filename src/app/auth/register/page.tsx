...
export default function RegisterPage() {
  ...
  return (
    <div className="mx-auto max-w-md py-4 md:py-16">
      ...
      <p className="mt-1 text-sm text-mut">Создайте аккаунт, чтобы искать попутчиков.</p>
      <div className="mt-6 space-y-4 rounded-2xl border border-line bg-white p-6 shadow-card">
        <div>
          <label className="label">Имя *</label>
          <input className="input" required value={name} onChange={(e) => setName(e.target.value)} placeholder="Айбол" />
        </div>
        <div>
        ...
        <p className="text-center text-sm text-mut">
          Уже есть аккаунт? <Link href="/auth/login" className="font-semibold text-route">Войти</Link>
        </p>
      </div>
